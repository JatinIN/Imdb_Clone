...
const displayMovieDetails = (details) => {
    ...
    <img src="${(details.Poster != 'N/A') ? details.Poster : '../Assets/NO_Image.png'}" alt="movie-poster">
</div>
...
<div class="movie-info">
    ...
    <p class="language"><b><i>Language: </b> ${details.Language}</i></p>
...
async function getData(movieID) {
    const BASEURL = `https://www.omdbapi.com/?i=${movieID}&apikey=76ef9c9f`;
    const result = await fetch(BASEURL); //Base URL
    const movieDetails = await result.json(); //Movie Details from server
    displayMovieDetails(movieDetails); //Display the movie

}

//Set button text to already added if it is there in fav-list
if(movieID){
if(favMovies.includes(movieID)){
    addToFavBtn.textContent = 'Already In Favourites';
}
}
